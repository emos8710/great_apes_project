def Incoherence_filter(Cov_result, incoherenceThreshold, incoherenceThresholdSmooth, binsize=3, jumpSize=100):
    import numpy as np
    ...
    for x in Cov_result:
        ...
        for b in range(len(bins)):
            if bins[b] < incoherenceThresholdSmooth:  # Save bins with low incoherence
                for d in range(numlengths[b]):
                    ...
                    FilteredRefSmooth.append(reference[position + d])
                    FilteredMapSmooth.append(mapped[position + d])
        ...
        result[x]['positions__smooth'] = FilteredPositionsSmooth
        result[x]['mapped_nucs_smooth'] = FilteredMapSmooth
        result[x]['ref_nuc_smooth'] = FilteredRefSmooth
    return result