...
for i in range(len(files)):
    ...
    Cov_result = map_percent_filter(mapped, MappedThreshold, virus_sizes)  # Filter out viruses with less than given mapped threshold.
    # This is to reduce the amount of viruses to speed up computation
...
with open(output, 'wt') as outfile:
    ...
    for i in range(len(files)):  # go through all files and call functions A, B and C.
        ...
        for key in Abundance:  # Go through all virus IDs, should be same from all functions from the same file
            if (Abundance.get(key) > AbundanceThreshold and Coverage.get(key) < CoverageThreshold) and ErrorRate.get(key) < ErrorRateThreshold:
                for id in virus:
                    if id == key:
