	...
	for i in range(len(files)):  # Go through all files and call functions A, B and C.
		...
		for key in Abundance:  # Go through all virus IDs, should be same from all functions from the same file
			...
			if Abundance.get(key) > AbundanceThreshold and Coverage.get(key)[3] == 1 and Coverage.get(key)[0] < CoverageThreshold and ErrorRate.get(key)[0] < ErrorRateThreshold:
				#keptViruses = {'Abundance': Abundance.get(key), 'Coverage': 1 - Coverage.get(key)[0], 'ErrorRate':
				# ErrorRate.get(key)}
...
# Ape dictionary has file names as keys and values are dictionaries with virus IDs as keys. This inner dictionary has a
# dictionary containing the attributes of the virus. Example with two ape files whre the first one contains one virus
# and the second one contains two viruses:
# {'sample_data/testapa.mpile.gz': {'NC_014069.1': {'Abundance': 3.8292282430213467, 'ErrorRate': [0.08404802744425385],
# 'Coverage': 0.3300813008130081}}, 'sample_data/Gorilla_beringei_beringei-Umurimo.mpile.gz': {'NC_008913.1':
# {'Abundance': 5.430722891566265, 'ErrorRate': [0.04991680532445923], 'Coverage': 0.10516312955337348}, 'NC_009889.1':
# {'Abundance': 12.477586206896552, 'ErrorRate': [0.1494058311455023], 'Coverage': 0.2759933380918391}}}
