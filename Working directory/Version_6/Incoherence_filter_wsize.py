def incoherence_filter_sw(Cov_result, incoherenceThreshold, smooth, binsize=3, jumpSize=100):
    ...
    for x in Cov_result:
        ...
        Copy_fractions = list(FilteredFractionsSmooth)
        removed = [i for i in Fractions if not i in Copy_fractions or Copy_fractions.remove(i)]
        zeros = removed.count(0)
        nonzeros = 0
        for r in FilteredFractionsSmooth:
            if r > 0:
                nonzeros = nonzeros + 1

        if len(deleteSites) > 0:
            result.append((float(zeros) / (float(len(deleteSites))))*nonzeros)
            delete.append(float(zeros) / (float(len(deleteSites))))
        zero.append(zeros)
        nonzero.append(nonzeros)


    finalResult = np.mean(result)
    delete_final = np.mean(delete)
    return finalResult, zero, nonzero, delete_final